...
void loop() {
  ...
  error=mySerial.read(); //Read the serial data and store it
  Serial.print(" error=");
  Serial.println(error);
  http.begin(client,"http://192.168.43.35/test/designv4.php?ToCo_id="+ToCo_id+"&compartment="+compartment+"&area="+area+"error="+error);
  delay(500);
...
//  }
  ...
  card="xxxxxxxx";
  point=-1;
   http.begin(client,"http://192.168.43.35/test/designv4.php?ToCo_id="+ToCo_id+"&gas="+gas+"&count1="+count1+"&water="+water+"&area="+area+"&compartment="+compartment+"&point="+point+"&card="+card);
   delay(500);
   int httpCode = http.GET();
   Serial.println(httpCode);
   http.end();
